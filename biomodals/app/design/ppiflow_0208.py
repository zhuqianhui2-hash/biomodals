...
### Partial flow antibody / nanobody args (sample_antibody_partial_flow.py)
...
- Runs volume: `ppiflow-runs` mounted at `/runs`

Expected checkpoint layout (one-time upload examples):

  modal volume put ppiflow-models /path/to/antibody.ckpt antibody.ckpt
  modal volume put ppiflow-models /path/to/binder.ckpt   binder.ckpt
  modal volume put ppiflow-models /path/to/monomer.ckpt  monomer.ckpt
  modal volume put ppiflow-models /path/to/nanobody.ckpt nanobody.ckpt
...
TASK_TO_SCRIPT = {
    ...
    "ab_partial_flow": f"{PPIFLOW_DIR}/sample_antibody_partial_flow.py",
    "nb_partial_flow": f"{PPIFLOW_DIR}/sample_antibody_partial_flow.py",
...
def run_ppiflow_structured(
    ...
    # ---------- partial flow antibody/nanobody (sample_antibody_partial_flow.py) ----------
    ...
    # Modified: fail fast on duplicate run names so old artifacts cannot leak into a new bundle.
    if run_dir.exists():
    ...
    # -------------------------
    # Task: partial flow antibody/nanobody (sample_antibody_partial_flow.py)
    ...
    if p.returncode != 0:
        # Modified: persist failure logs before raising so stdout.log/cmd.txt remain inspectable.
        RUNS_VOL.commit()
...
def submit_ppiflow(
    ...
    # scaffolding motif csv
    if task == "scaffolding":
        if not scaffold_motif_csv:
            raise ValueError("scaffolding requires --scaffold-motif-csv")
        input_files.append(_read_file_as("motif.csv", scaffold_motif_csv))  # type: ignore[arg-type]
